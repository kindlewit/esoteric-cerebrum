...
} = require('../utils/question-utils');

export const createQuestions = async function (request, reply) {
  if (
    !request.body ||
    request.body === {} ||
  ...
  // Always insert multiple questions
  try {
    // Returning bad request immediately
    // let acceptableQns = request.body.filter(
    //   (qn) => qn.number && qn.text && qn.answer_format && qn.weightage
    // );
    // if (acceptableQns.length !== request.body.length) {
    //   return reply.code(400).send();
    // }
    ...
    // let finalQuestions = [];
    // let finalOptions = [];
    // for (let i = 0; i < acceptableQns.length; i++) {
    //   let { options, answer, ...qn } = acceptableQns[i];
    ...
    let questions = request.body
      .filter((qn) => qn.number && qn.text && qn.answer_format && qn.weightage)
      .map((qn) => ({ ...qn, username, three_words: threeWords }));
  } catch (e) {
    request.log.error(e);
