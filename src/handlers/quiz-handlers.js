...
import Quiz from '../services/quiz-services';
import Question from '../services/question-services';
import db from '../orm';
import { generateQuizQRCode, computePagination } from '../utils/misc-utils';
import { verifyUserAuthority, fetchDisplayNameFor } from '../utils/user-utils';
import { autoEvaluatable } from '../utils/question-utils';
...
async function listQuizHandler(request, reply) {
  try {
    if (_.has(request.query, 'count') && request.query.count) {
      ...
      let page = _.has(request.query, 'page')
        ? parseInt(request.query.page)
        : null;
      let size = _.has(request.query, 'size')
...
async function collateQuizHandler(request, reply) {
  ...
  try {
    ...
    let { threeWords } = request.params;
    let quizDoc = await Quiz.get(threeWords);
    if (_.isEmpty(quizDoc)) {
      return reply.code(404).send();
    }
    let questions = await Question.ofQuiz(threeWords, { options: true });
    return reply.code(200).send({ ...quizDoc, questions });
  } catch (e) {
    request.log.error(e);
    return reply.code(500).send();
  }
